...
def upgrade():
    ...
    op.execute("CREATE TYPE componentstate AS ENUM " "('PENDING', 'INDEXING', 'ANALYZING', 'COMPLETED', 'ERROR')")
    op.execute(
        "ALTER TABLE session_component ADD COLUMN state componentstate " "NOT NULL DEFAULT 'PENDING'::componentstate"
    )
...
def downgrade():
    ...
    op.execute("ALTER TABLE session ADD COLUMN state sessionstate " "NOT NULL DEFAULT 'PENDING'::sessionstate")
    op.drop_column('session_component', 'indexed_asset_id')
    op.drop_column('session_component', 'state')
    op.drop_table('twelve_labs_index')
    postgresql.ENUM('PENDING', 'INDEXING', 'ANALYZING', 'COMPLETED', 'ERROR', name='componentstate').drop(
